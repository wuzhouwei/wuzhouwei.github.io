...
```html
<body>
  <div id="idSelection">id选择器</div>
  <div class="classSelection">其他选择器</div>

  <script>
    //暂时推荐两种
    const idSelection = document.getElementById("idSelection");
    const classSelection = document.querySelectorAll(".classSelection")[0];
    console.log(idSelection, "idSelection", classSelection, "classSelection");
  </script>
</body>
```
...
> `classList` 比 `className` 更灵活且更推荐用于对类名进行操作。

```html
<body>
  <div id="test1">id选择器</div>
  <div id="test2">id选择器</div>

  <script>
    const test1 = document.getElementById("test1");
    const test2 = document.getElementById("test2");
    //类名操作 className
    test1.className = "addClass"; //单个类名
    test1.className = "addClass addClass2"; //多个类名
    test1.className = ""; //去除类名

    //类名操作 classList
    test2.classList.add("addClass"); //单个类名
    test2.classList.add(...["addClass", "addClass2"]); //多个类名
    test2.classList.toggle("addClass2"); //切换，有则移除，没有则添加
    test2.classList.remove("addClass");
    const isContains = test2.classList.contains("addClass"); //判断是否存在该class
...
### 4.innerHTML or innerText

> `innerHTML`可以解析标签, `innerText`不可以解析标签
...
<body>
  ...
  <script>
    ...
    test.innerHTML = `<p>hello</p>`; //可以解析标签
    test.innerText = `<p>hello</p>`; //不可以解析标签
  </script>
</body>
```
...
<body>
  ...
  <script>
    ...
    const text1 = document.createTextNode("text1"); //创建第一个节点
    const text2 = document.createTextNode("text2"); //创建第二个节点
    newDiv.appendChild(text1); //加入节点
    newDiv.appendChild(text2); //加入节点
    newDiv.removeChild(text2); //移除节点
    document.body.appendChild(newDiv); //把创建的加入到body元素中
  </script>
</body>
```
